<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>기업 정보</title>
    <style>
        body {
            font-family: Pretendard-Regular, Arial, sans-serif;
            background-color: #f5f5f5;
            display: flex;
            justify-content: center;
            align-items: center;
            height: 100vh;
            margin: 0;
            padding-top: 20px;
        }
        .container {
            background-color: #fff;
            border-radius: 10px;
            box-shadow: 0 0 20px rgba(0, 0, 0, 0.1);
            width: 600px;
            padding: 20px;
            box-sizing: border-box;
        }
        .container h2 {
            margin-top: 0;
            text-align: center;
        }
        .info-group {
            display: flex;
            flex-direction: column;
            margin-bottom: 10px;
        }
        .info-group label {
            font-weight: bold;
        }
        .info-group p {
            margin: 0;
        }
        .button-group {
            display: flex;
            justify-content: space-around;
            margin-top: 20px;
        }
        .button-group button {
            background-color: #007bff;
            color: #fff;
            padding: 10px 20px;
            font-size: 16px;
            border: none;
            border-radius: 5px;
            cursor: pointer;
        }
        .button-group button:hover {
            background-color: #0056b3;
        }
        .button-group .delete {
            background-color: #dc3545;
        }
        .button-group .delete:hover {
            background-color: #c82333;
        }
    </style>
</head>
<body>
    <div class="container">
        <h2>기업 정보</h2>
        <div class="info-group">
            <label for="bid">사업자번호</label>
            <p>${business.bid}</p>
        </div>
        <div class="info-group">
            <label for="bpw">비밀번호</label>
            <p>${business.bpw}</p>
        </div>
        <div class="info-group">
            <label for="bname">회사명</label>
            <p>${business.bname}</p>
        </div>
        <div class="info-group">
            <label for="address">주소</label>
            <p>${business.address}</p>
        </div>
        <div class="info-group">
            <label for="salary">급여</label>
            <p>${business.salary}</p>
        </div>
        <div class="info-group">
            <label for="welfare">복리후생</label>
            <p>${business.welfare}</p>
        </div>
        <div class="info-group">
            <label for="ceo">CEO</label>
            <p>${business.ceo}</p>
        </div>
        <div class="info-group">
            <label for="sales">매출</label>
            <p>${business.sales}</p>
        </div>
        <div class="info-group">
            <label for="employees">직원 수</label>
            <p>${business.employees}</p>
        </div>
        <div class="info-group">
            <label for="type">유형</label>
            <p>${business.type}</p>
        </div>
        <div class="info-group">
            <label for="industry">산업</label>
            <p>${business.industry}</p>
        </div>
        <div class="info-group">
            <label for="detailIndustry">세부 산업</label>
            <p>${business.detailIndustry}</p>
        </div>
        <div class="info-group">
            <label for="homepage">홈페이지</label>
            <p>${business.homepage}</p>
        </div>
        <div class="info-group">
            <label for="content">설명</label>
            <p><c:out value="${business.content}" escapeXml="false" /></p>
        </div>
        <c:if test="${sessionScope.bid eq business.bid}">
        <div class="button-group">
            <a href="${pageContext.request.contextPath}/business/businessupdate"><button onclick="location.href='businessUpdate.jsp'">수정하기</button></a>
            <a href="${pageContext.request.contextPath}/business/main"><button onclick="location.href='main.jsp'">홈으로</button></a>
            <a href="${pageContext.request.contextPath}/business/businessdelete"><button class="delete" onclick="location.href='businessDelete.jsp'">탈퇴하기</button></a>
        </div>
        </c:if>
        
        <c:if test="${sessionScope.bid ne business.bid}">
        <div class="button-group">
            <a href="${pageContext.request.contextPath}/business/main"><button onclick="location.href='main.jsp'">홈으로</button></a>
        </div>
        </c:if>
    </div>
</body>
</html>
