...
.resume-header {
	...
	/* Added thick border below resume-header */
	padding-bottom: 10px;
}

.resume-header img {
	border-radius: 50%;
	width: 200px; /* Increased size by 30% from 100px */
	height: 200px; /* Increased size by 30% from 100px */
	object-fit: cover;
}

.resume-header .name {
	font-size: 31px; /* Increased size by 30% from 24px */
	font-weight: bold;
}
...
.section {
	...
	border-bottom: 3px solid #000;
	/* Added thick border below each section */
	padding-bottom: 10px;
}
...
.section-title {
	font-size: 18px;
	font-weight: bold;
	width: 35%; /* Increased width by 20% from 150px */
	margin-top: 8px; /* Adjusted to align with table content */
}

.section-content {
	flex-grow: 1;
	border-left: none; /* Remove border between title and content */
}
...
<body>
	<aside class="sidebar">
		...
		<ul>
			...
			<li class="mb-2"><a href="#">서류 추천</a></li>
			<li class="mb-2"><a href="#">추가정보</a></li>
			<li class="mb-2"><a href="#">사전질문</a></li>
			<li class="mb-2"><a href="#">설문 응답</a></li>
			<li class="mb-2"><a
				href="${pageContext.request.contextPath}/anno/business-anno-management?annoId=${anno.annoId}">이력서
					관리</a></li>
		</ul>
	</aside>
	...
	<div class="review-panel">
		<form name="evaluate"
			...
			<div class="button-container">
				...
				<button type="button" onclick="nextStage(`${anno.annoId}`,`${resume.resumeId}`,`${resumeColumnStage}`)" >합격</button>
			</div>
		</form>
	</div>
	...
	<script>
		...
		function navigateToList(annoId) {
			window.location.href = "${pageContext.request.contextPath}/anno/business-anno-management?annoId="+annoId;
		}
		
		function nextStage(annoId,resumeId,resumeColumnStage) {
			window.location.href = "${pageContext.request.contextPath}/anno/anno-resume-nextstage?annoId="+annoId+"&resumeId="+resumeId+"&resumeColumnStage="+resumeColumnStage;
		}
